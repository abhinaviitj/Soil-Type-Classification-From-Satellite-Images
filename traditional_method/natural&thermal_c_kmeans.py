...
def return_glcm(folder) :
    ...
    for i in range(0,40,2):
        ...
        for j in range(len(img)):
            glcm = greycomatrix(img[j], [1], [0, np.pi/4, np.pi/2, 3*np.pi/4], levels = None)
            glcm = glcm[:,:,0,0]
            fet=fet+list(glcm)
        # print(i)
...
f=open("res_natural&thermal_c_kmeans.txt", "a+")
f.write("\n")

lst = ["alluvial","black","desert","red"]
f.write("result when clustering is on only natural colour and thermal band with color as feature:\n")
for j in range(4):
    k=j*20
